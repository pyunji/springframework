...
public class Pager {
	...
	public Pager(int rowsPerPage, int pagesPerGroup, int totalRows, int pageNo) {
		...
		startRowNo = (pageNo - 1) * rowsPerPage + 1; // oracle에서 사용
		startRowIndex = startRowNo - 1; // mysql에서 사용
		endRowNo = pageNo * rowsPerPage; // oracle에서 사용
		endRowIndex = endRowNo - 1; // mysql에서 사용
	}
