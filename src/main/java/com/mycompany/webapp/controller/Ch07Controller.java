...
public class Ch07Controller {
	...
	public String saveData(HttpServletRequest request) {
		...
		// Request 범위에 데이터를 저장하기 -> 전에 궁금했던 그 상황에 적용되는 것인가?
		request.setAttribute("requestData", "자바");
		// /readData를 요청하게 되면 새로운 요청이 들어오는 것이므로 해당 데이터는 사라진다.
		
		// Session 범위에 데이터를 저장하기
		HttpSession session = request.getSession();
		session.setAttribute("sessionData", "자바스크립트");
		// /readData를 요청하게 되면 동일한 브라우저에서 요청하는 것이므로 해당 데이터가 그대로 남아있다.
	...
	public String useJstl2(Model model) {
		...
		model.addAttribute("boardList", list); // request 범위에 저장함
		
		return "ch07/useJstl2";
	}
	
	@ModelAttribute ("colors") // request 범위에서만 사용 가능
	...
	public String argumentSaveAndRead2(@ModelAttribute("cloth") Ch07Cloth cloth) { // 어노테이션 안에 이름을 지정하지 않으면 기본으로 클래스명의 앞글자만 소문자로 바꿔서 등록된다.
		logger.info("실행");
