...
public class Ch04MemberJoinFormValidator implements Validator{
	...
	@Override
	public boolean supports(Class<?> clazz/*dto 클래스 하나 대입*/) {	// 주어진 데이터를 유효성 검사할 수 있는지 여부 반환
		logger.info("실행");
		boolean check = Ch04Member.class.isAssignableFrom(clazz); // clazz가 Ch04Member.class로 대입될 수 있느냐 (대입: A b = new B();)
		return check;
	}
	...
	public void validate(Object target, Errors errors) {
		...
		if(member.getMpassword() == null || member.getMpassword().trim().equals("")) {
			...
			if(member.getMpassword().length() < 8) {
				errors.rejectValue("mid", "errors.mpassword.minlength", new Object[] {8}, "");
			}
		}
